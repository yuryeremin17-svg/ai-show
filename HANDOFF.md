...
Пилотная серия S01E01 "Обратитесь к дизайнеру" — раскадровка готова, ждёт картинки и озвучку.
...
## Что дальше
1. **ElevenLabs API key** → подключить генерацию голосов (Юрий, Клодище, Today)
2. **OpenAI API key** (опционально) → автогенерация картинок через DALL-E
3. **Картинки для S01E01** → Midjourney или DALL-E, 10 кадров → `assets/scenes/S01E01/`
4. **Музыка** → Suno AI → интро-джингл + фон → `assets/music/`
5. **Первая сборка S01E01** → `python3 scripts/assemble.py S01E01`
6. **Скилл /episode** (после обкатки) → одна команда = весь цикл
